...
#define DEFINE_JIT_STRUCT_HELPER_MEMBER_INIT(r, data, i, elem)                                                         \
  {                                                                                                                    \
    ...
    using FT2 = typename std::remove_const<std::remove_pointer<typename std::remove_reference<FT>::type>::type>::type; \
    ...
    if constexpr (std::is_same<FT2, char>::value || std::is_same<FT2, bool>::value ||                                  \
                  ...
                  std::is_same<FT, const flatbuffers::String *>::value || std::is_same<FT, const char *>::value) {     \
...
#define DEFINE_JIT_STRUCT_HELPER(st, ...)                                                                     \
  ...
  struct JitStructHelper<st> {                                                                                \
    ...
    static void InitJitBuilder() {                                                                            \
      ...
      BOOST_PP_SEQ_FOR_EACH_I(DEFINE_JIT_STRUCT_HELPER_MEMBER_INIT, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
    }                                                                                                         \
  };                                                                                                          \
  }
