...
DEFINE_EXPR_STRUCT(Combine, (User *)user, (Item *)item, (double)score,
                   (std::string)name)
...
TEST(ExprTest, Logic) {
  ...
  //                         true                true                  false
  int rc = expr.Init(
      "user.id == \"uid123\" && (item.score > 1 || item.id == \"item101\") ",
      opt);
  EXPECT_EQ(0, rc);
...
TEST(ExprTest, Calc) {
  ...
  opt.Init<Combine>();
  int rc = expr.Init(
      "1 + 2.11 + hash(1712, 100) + user.score + (user.id == \"uid123\"?10:20)",
      opt);
  EXPECT_EQ(0, rc);
  SubUser su = {101, "random"};
  User user = {1.21, "uid123", &su};
...
TEST(ExprTest, DynamicVar) {
  ...
  dynamic_vars["v1"] = 101;
  opt.dynamic_var_access =
      [](const void *root,
         const std::vector<std::string> &args) -> ssexpr::Value {
    std::map<std::string, int64_t> &dynamic_vars =
        *((std::map<std::string, int64_t> *)root);
