...
typedef std::variant<bool, char, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t, uint64_t, float, double,
...
struct FieldAccessorTable
    : public std::map<std::string, std::variant<FieldAccessor, std::pair<FieldAccessor, FieldAccessorTable>>> {};
...
template <typename T>
int GetFieldAccessors(const std::vector<std::string> &names, std::vector<FieldAccessor> &accessors) {
  accessors.clear();
...
struct ExprStructHelper {
  ...
  static int GetFieldAccessors(const std::vector<std::string> &names, std::vector<FieldAccessor> &accessors) {
...
template <typename T>
struct is_container : std::integral_constant<bool, HasConstIterator<T>::value && HasBeginEnd<T>::beg_value &&
...
FieldValue toFieldValue(const T &v) {
  ...
  if constexpr (std::is_same<T, double>::value || std::is_same<T, float>::value || std::is_same<T, char>::value ||
                std::is_same<T, uint8_t>::value || std::is_same<T, int32_t>::value ||
                std::is_same<T, uint32_t>::value || std::is_same<T, int16_t>::value ||
                std::is_same<T, uint16_t>::value || std::is_same<T, int64_t>::value ||
                std::is_same<T, uint64_t>::value || std::is_same<T, std::string_view>::value ||
...
bool fillHelperFieldAccessorTable(FieldAccessorTable &v) {
  ...
  if constexpr (std::is_same<RR, double>::value || std::is_same<T, float>::value || std::is_same<RR, char>::value ||
                std::is_same<T, uint8_t>::value || std::is_same<RR, int32_t>::value ||
                std::is_same<T, uint32_t>::value || std::is_same<RR, int16_t>::value ||
                std::is_same<T, uint16_t>::value || std::is_same<RR, int64_t>::value ||
                std::is_same<T, uint64_t>::value || std::is_same<RR, std::string_view>::value ||
                std::is_same<T, bool>::value || std::is_same<RR, std::string>::value ||
...
#define DEFINE_EXPR_STRUCT(st, ...)                                                                      \
  struct st {                                                                                            \
    ...
    inline ssexpr::FieldValue GetFieldValue(const std::vector<ssexpr::FieldAccessor> &accessors) const { \
