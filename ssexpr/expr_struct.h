...
#include "ssexpr/expr_macro.h"

namespace ssexpr {

typedef std::variant<bool, char, uint8_t, int16_t, uint16_t, int32_t, uint32_t,
                     int64_t, uint64_t, float, double, std::string_view,
                     const void *>
    FieldValueVariant;

typedef FieldValueVariant FieldValue;
template <typename T, typename R> inline T GetValue(R &v) {
  return std::get<T>(v);
}
...
struct FieldAccessorTable
    : public std::map<
          std::string,
          std::variant<FieldAccessor,
                       std::pair<FieldAccessor, FieldAccessorTable>>> {};

inline FieldValue GetFieldValue(const void *root,
                                const std::vector<FieldAccessor> &accessors) {
  const void *obj = root;
  FieldValue empty;
      ...
      if (i == names.size() - 1) {
        ...
        try {
          std::pair<FieldAccessor, FieldAccessorTable> *accessor =
              std::get_if<std::pair<FieldAccessor, FieldAccessorTable>>(
                  &found->second);
          if (nullptr == accessor) {
            return -1;
          }
...
template <typename T> struct ExprStructHelper {
  ...
  template <typename fake = void> static int InitExpr();
};
...
// SFINAE test
template <typename T> class HasGetFieldAccessors {
  typedef char one;
  struct two {
    char x[2];
  };

  template <typename C> static one test(decltype(&C::GetFieldAccessors));
...
template <typename T>
struct is_container
    : std::integral_constant<bool, HasConstIterator<T>::value &&
                                       HasBeginEnd<T>::beg_value &&
...
template <typename T> FieldValue toFieldValue(const T &v) {
  using R =
  ...
  if constexpr (std::is_same<T, double>::value ||
    ...
    return rv;
  } else if constexpr (is_container<R>::value ||
                       std::is_base_of<NoExpr, R>::value) {
...
template <typename T> bool fillFieldAccessorTable(FieldAccessorTable &v) {
  using R =
      typename std::remove_const<typename std::remove_pointer<T>::type>::type;
  if constexpr (std::is_same<T, double>::value ||
                std::is_same<T, float>::value || std::is_same<T, char>::value ||
...
template <typename T> bool fillHelperFieldAccessorTable(FieldAccessorTable &v) {
  ...
  if constexpr (std::is_same<RR, double>::value ||
                std::is_same<T, float>::value ||
                std::is_same<RR, char>::value ||
                std::is_same<T, uint8_t>::value ||
                std::is_same<RR, int32_t>::value ||
                std::is_same<T, uint32_t>::value ||
                std::is_same<RR, int16_t>::value ||
                std::is_same<T, uint16_t>::value ||
                std::is_same<RR, int64_t>::value ||
                std::is_same<T, uint64_t>::value ||
                std::is_same<RR, std::string_view>::value ||
                std::is_same<T, bool>::value ||
...
#define FIELD_EACH(N, i, arg)                                                  \
  PAIR(arg){};                                                                 \
  template <typename fake> struct FieldInit<i, fake> {                         \
    FieldInit(ssexpr::FieldAccessorTable &accessors) {                         \
      ...
      using R = typename std::remove_const<                                    \
          typename std::remove_pointer<DT>::type>::type;                       \
...
#define FIELD_EACH_INIT(N, i, arg)                                             \
  static FieldInit<i, void> __init__##N(GetFieldAccessorTable());

#define DEFINE_EXPR_STRUCT(st, ...)                                            \
  struct st {                                                                  \
    typedef st __CurrentDataType;                                              \
    template <int N, typename fake> struct FieldInit {};                       \
    static ssexpr::FieldAccessorTable &GetFieldAccessorTable() {               \
      static ssexpr::FieldAccessorTable accessors;                             \
      return accessors;                                                        \
    }                                                                          \
    static int                                                                 \
    GetFieldAccessors(const std::vector<std::string> &names,                   \
                      std::vector<ssexpr::FieldAccessor> &accessors) {         \
      return ssexpr::GetFieldAccessors<st>(names, accessors);                  \
    }                                                                          \
    inline ssexpr::FieldValue                                                  \
    GetFieldValue(const std::vector<ssexpr::FieldAccessor> &accessors) const { \
    ...
    (FIELD_EACH, 0, __VA_ARGS__)                                               \
      ...
      PASTE(REPEAT_, GET_ARG_COUNT(__VA_ARGS__))                               \
      (FIELD_EACH_INIT, 0, __VA_ARGS__) once = true;                           \
      return;                                                                  \
    }                                                                          \
  };
