...
struct Operand
    : public x3::variant<Nil, int64_t, double, bool, std::string, Variable,
                         DynamicVariable, x3::forward_ast<FuncCall>,
                         x3::forward_ast<Unary>, x3::forward_ast<CondExpr>,
...
x3::rule<class identifier_rule_, std::string> const identifier_rule =
    "identifier_rule";
...
auto make_conditional_op = [](auto &ctx) {
  ...
  x3::_val(ctx) = n;
  // x3::_val(ctx) =
  //     ssexpr::ast::CondExpr{x3::_val(ctx), at_c<0>(x3::_attr(ctx)),
  //     at_c<1>(x3::_attr(ctx))};
};

auto const
    cond_expr_def = logical_expr[([](auto &ctx) { _val(ctx) = _attr(ctx); })] >>
...
boost::spirit::x3::real_parser<
    double, boost::spirit::x3::strict_real_policies<double>> const
    strict_double_ = {};
const auto int_or_double = strict_double_ | boost::spirit::x3::int64;
auto const primary_expr_def = int_or_double | bool_ | quoted_string | func |
                              dynamic_var | var | '(' > expression > ')';

auto const expression_def = cond_expr;

BOOST_SPIRIT_DEFINE(expression, cond_expr, logical_expr, equality_expr,
                    relational_expr, additive_expr, multiplicative_expr,
                    unary_expr, func, primary_expr);
    ...
    case op_equal: {
      ...
      if constexpr ((std::is_same<decltype(left), const int64_t &>::value &&
                    ...
                    (std::is_same<decltype(left), const double &>::value &&
                     (std::is_same<decltype(right), const double &>::value ||
                      std::is_same<decltype(right), const int64_t &>::value)) ||
                    (std::is_same<decltype(left),
                                  const std::string_view &>::value &&
                     std::is_same<decltype(right),
                                  const std::string_view &>::value) ||
...
struct Initializer {
  ...
  int operator()(Unary &n) const {
    return boost::apply_visitor(*this, n.operand_);
  }
...
namespace ssexpr {
int SpiritExpression::Init(const std::string &expr,
                           const ExprOptions &options) {
  options_ = options;
